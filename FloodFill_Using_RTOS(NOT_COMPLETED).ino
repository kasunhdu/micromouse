...
void setup() {
 ...
 //floodfill queue and task
  ...
  xTaskCreate(FloodFillTask, "TheFloodFillTask", 10000, NULL, 1, NULL);

  xTaskCreate(task_MOTOR_Control, "Task_MOTOR_Control", 7000, NULL, 2, &MOTOR_Control_TaskHandle);
  xTaskCreate(MOTOR_Task, "MOTORTask", 1000, NULL, 2, NULL);
 
  xTaskCreate(task1_LEFT_Sharp_IR, "Task1_LEFT_Sharp_IR", 2000, NULL, 0, &task1HandleLEFTsHARRPIR);
  xTaskCreate(task2_RIGHT_Sharp_IR, "Task2_RIGHT_Sharp_IR", 2000, NULL, 0, &task2HandleRIGHTsHARRPIR);
  xTaskCreate(task3_MAIN_Sharp_IR, "Task3_MAIN_Sharp_IR", 2000, NULL, 0, &task3HandleMainSHARRP_IR);
